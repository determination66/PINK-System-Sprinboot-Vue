            ...
            <el-col :span="24">
              ...
              <div class="recommend-content">
                <div class="recommend-title">
                  <span class="title_1" :class="{ 'active': isShow }" @click="isShow = true">推荐职位</span>
                  <span class="title_2" :class="{ 'active': !isShow }" @click="isShow = false">申请详情</span>
                </div>
                ...
                <div class="recommend-list">
                  ...
                  <div v-else>
                    <!-- <h1>else</h1> -->
                    <!-- <post-item v-for="post in posts" :key="post.index" :post="post"></post-item> -->
                    <ApplyProcess v-for="apply in applys" :key="apply.index" :apply="apply"></ApplyProcess>
                  </div>
...
<script>
import ApplyProcess from '../../components/apply-system/post/ApplyProcess.vue';
import PostItem from "../../components/apply-system/post/PostItem.vue";
import AnnexPanel from "../../components/apply-system/profile/AnnexPanel.vue";

export default {
  name: "profile",
  components: {AnnexPanel, PostItem,ApplyProcess},
  data() {
    return {
      uploadData:{
  ...
  created(){
    ...
    this.$http.get('http://localhost:8085/index/apply_edit/'+localStorage.getItem('c_id'))
      ...
      .catch(function (error) {
        ...
        //通过用户id获取申请信息
        this.$http.get('http://localhost:8085/index/apply_home/applyProcess/'+localStorage.getItem('c_id'))
      .then(function (response) {
      for(var len = 0;len<response.data.length;len++){
        ...
        v.applys.push(response.data[len])
        }
      },
      )
      .catch(function (error) {
